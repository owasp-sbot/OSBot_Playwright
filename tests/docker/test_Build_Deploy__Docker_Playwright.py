...
from osbot_playwright.docker.Build_Deploy__Docker_Playwright import Build_Deploy__Docker_Playwright


#@pytest.mark.skip("review: once new version playwright docker lambda is created")
class test_Build_Deploy__Docker_Playwright(TestCase):
    ...
    def test__init__(self):
        ...
        deploy_lambda     = self.build_deploy.deploy_lambda
        docker_image      = create_image_ecr.docker_image
        docker_image_name = f'{self.aws_account_id}.dkr.ecr.eu-west-2.amazonaws.com/{self.build_deploy.image_name}'

        assert self.build_deploy.image_name                  == 'osbot_playwright'
        assert self.build_deploy.path_images                 == create_image_ecr.path_images
    ...
    def test_path_docker_playwright(self):
        ...
        assert folder_name  (self.build_deploy.path_docker_playwright()) == 'osbot_playwright'
